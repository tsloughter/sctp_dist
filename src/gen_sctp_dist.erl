...
-export([dist_cntrlr_setup/2, dist_cntrlr_input_setup/4,
...
listen(Name) ->
    case do_listen([binary, {active, false}, {reuseaddr, true}]) of
	{ok, Socket} ->
...
do_listen(Options) ->
    ...
    do_listen(First, Last, listen_options(Options)). %% {backlog,128}

do_listen(First,Last,_) when First > Last ->
    {error,eaddrinuse};
do_listen(First,Last,Options) ->
    case gen_sctp:open(First, [{recbuf,65536}, {mode,binary} | Options]) of
	{error, eaddrinuse} ->
	...
	{ok, Socket} ->
            case gen_sctp:listen(Socket, true) of
                {error, eaddrinuse} ->
                    do_listen(First+1, Last, Options);
                ok ->
                    %% ?trace("listening on ~p ~p", [First, Options]),
                    {ok, Socket}
            end
    end.
...
accept_loop(Kernel, Listen) ->
    ...
    case gen_sctp:recv(Listen) of
        {ok, {_, _, [], NewAssoc=#sctp_assoc_change{}}} ->
            {ok, Socket} = gen_sctp:peeloff(Listen, NewAssoc),
            DistCtrl = spawn_dist_cntrlr(Socket, NewAssoc),
            ?trace("~p~n",[{?MODULE, accept_loop, accepted, Socket, DistCtrl, self()}]),
	    %% flush_controller(DistCtrl, Socket),
	    gen_sctp:controlling_process(Socket, DistCtrl),
	    %% flush_controller(DistCtrl, Socket),
	    Kernel ! {accept,self(),DistCtrl,inet,sctp},
            receive
...
flush_controller(Pid, Socket) ->
    receive
	{sctp, Socket, Peer, Port, Data} ->
	    Pid ! {sctp, Socket, Peer, Port, Data},
	    flush_controller(Pid, Socket)
    after 0 ->
	    ok
    end.
...
do_setup(Kernel, Node, Type, MyNode, LongOrShortNames, SetupTime) ->
	...
	{ok, Ip} ->
		...
		{port, SctpPort, Version} ->
		    ?trace("port_please(~p) -> version ~p~n", 
			   [Ip,Version]),
		    dist_util:reset_timer(Timer),
                    {ok, Socket} = gen_sctp:open([{mode,binary}]),
                    case gen_sctp:connect(Socket, Ip, SctpPort,
                                          [{active, false},{mode,binary},
                                           {sctp_initmsg,#sctp_initmsg{num_ostreams=5}}]) of
			{ok, Assoc} ->
                            DistCtrl = spawn_dist_cntrlr(Socket, Assoc),
...
%% ticks.
%%
dist_cntrlr_tick_handler(Socket, Assoc) ->
    receive
        tick ->
            %% May block due to busy port...
            sock_send(Socket, Assoc, "");
        _ ->
            ok
    end,
    dist_cntrlr_tick_handler(Socket, Assoc).

spawn_dist_cntrlr(Socket, Assoc) ->
    spawn_opt(?MODULE, dist_cntrlr_setup, [Socket, Assoc],
              [{priority, max}] ++ ?DIST_CNTRL_COMMON_SPAWN_OPTS).

dist_cntrlr_setup(Socket, Assoc) ->
    TickHandler = spawn_opt(?MODULE, dist_cntrlr_tick_handler,
                            [Socket, Assoc],
                            [link, {priority, max}] 
                            ++ ?DIST_CNTRL_COMMON_SPAWN_OPTS),
    dist_cntrlr_setup_loop(Socket, Assoc, TickHandler, undefined).
...
%%
dist_cntrlr_setup_loop(Socket, Assoc, TickHandler, Sup) ->
    receive
        %% {tcp_closed, Socket} ->
        ...
        {Ref, From, {supervisor, Pid}} ->
            Res = link(Pid),
            From ! {Ref, Res},
            dist_cntrlr_setup_loop(Socket, Assoc, TickHandler, Pid);
        ...
        {Ref, From, {send, Packet}} ->
            ...
            Res = gen_sctp:send(Socket, Assoc, 0, [<<Size:16>>, Packet]),
            From ! {Ref, Res},
            dist_cntrlr_setup_loop(Socket, Assoc, TickHandler, Sup);

        {Ref, From, {recv, _Length, Timeout}} ->
            case gen_sctp:recv(Socket, Timeout) of
                {ok, {_, _, [#sctp_sndrcvinfo{}], <<Size:16, Res:Size/binary>>}} ->
                    From ! {Ref, {ok, Res}},
                    dist_cntrlr_setup_loop(Socket, Assoc, TickHandler, Sup);
                _ ->
                    exit(connection_closed)
        ...
        {Ref, From, {address, Node}} ->
            Res = case inet:peernames(Socket, Assoc) of
                      {ok, [Address | _]} ->
                          case split_node(atom_to_list(Node), $@, []) of
                              [_,Host] ->
        ...
        {Ref, From, {handshake_complete, _Node, DHandle}} ->
            ...
            InputHandler = spawn_opt(?MODULE, dist_cntrlr_input_setup,
                                     [DHandle, Socket, Assoc, Sup],
                                     [link] ++ ?DIST_CNTRL_COMMON_SPAWN_OPTS),

	    flush_controller(InputHandler, Socket),
            gen_sctp:controlling_process(Socket, InputHandler),
	    flush_controller(InputHandler, Socket),
            ...
            dist_cntrlr_output_loop(DHandle, Socket, Assoc)
    end.
...
dist_cntrlr_input_setup(DHandle, Socket, Assoc, Sup) ->
    link(Sup),
    %% Ensure we don't try to put data before registerd
    %% as input handler...
    receive
        DHandle ->
            dist_cntrlr_input_loop(DHandle, Socket, Assoc, <<>>, 0)
    end.

dist_cntrlr_input_loop(DHandle, Socket, Assoc, Buffer, N) when N =< ?ACTIVE_INPUT/2 ->
    inet:setopts(Socket, [{active, ?ACTIVE_INPUT - N}]),
    dist_cntrlr_input_loop(DHandle, Socket, Assoc, Buffer, ?ACTIVE_INPUT);
dist_cntrlr_input_loop(DHandle, Socket, Assoc, Buffer, N) ->
    receive
        {sctp, _, _, _, {_, {sctp_shutdown_event, _}}} ->
            exit(connection_closed);

        %% {sctp, _, _, _, {_, #sctp_assoc_change{}}} -> %% ,shutdown_comp,0,0,0,133}}}
        %%     exit(connection_closed);
        {sctp, _, _, _, {_, R}} when is_tuple(R) ->
            %% Ignore...
            dist_cntrlr_input_loop(DHandle, Socket, Assoc, Buffer, N);
        {sctp, _, _, _, {_, Input}} ->
            case <<Buffer/binary, Input/binary>> of
                <<Size:32, Data:Size/binary, Rest/binary>> ->
                    try erlang:dist_ctrl_put_data(DHandle, Data)
                    catch _ : _ -> death_row()
                    end,
                    dist_cntrlr_input_loop(DHandle, Socket, Assoc, Rest, N-1);
                NewBuffer ->
                    dist_cntrlr_input_loop(DHandle, Socket, Assoc, NewBuffer, N-1)
            end;
        _ ->
            %% Ignore...
            dist_cntrlr_input_loop(DHandle, Socket, Assoc, Buffer, N)
...
dist_cntrlr_send_data(DHandle, Socket, Assoc) ->
    case erlang:dist_ctrl_get_data(DHandle) of
        none ->
            erlang:dist_ctrl_get_data_notification(DHandle);
        Data ->
            sock_send(Socket, Assoc, Data),
            dist_cntrlr_send_data(DHandle, Socket, Assoc)
    end.
...
dist_cntrlr_output_loop(DHandle, Socket, Assoc) ->
    receive
        dist_data ->
            ...
            try dist_cntrlr_send_data(DHandle, Socket, Assoc)
            catch _ : _ -> death_row()
            end,
            dist_cntrlr_output_loop(DHandle, Socket, Assoc);
...
sock_send(Socket, Assoc, Data) ->
    ...
    try gen_sctp:send(Socket, Assoc, 1, [<<Size:32>>, Data]) of
        ok -> ok;
